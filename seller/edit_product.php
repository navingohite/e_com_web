                ...
                <div class="outer-w3-agile mt-3">
                   ...
                   <form id="fupForm" method="post" enctype="multipart/form-data">
						...
						<div class="form-group col-md-6">
                                ...
                                <input type="text"  name="description" value="<?=$description?>"  class="form-control" id="">
                        </div>
					    </div>
						<div class="form-row">
...
?>
    ...
    <a href="upload/<?=$image?>"> <img class="img-fluid z-depth-1" src="../assets/upload/<?=$image?>" style="height:100px;width:150px" alt="video"
		...
		<button class="btn btn-info picdelete" picture_id="<?=$abc[0]?>">X
					...
					echo"</br></br>";	
					$rr=select("SELECT * FROM `size` where product_id='$pid'");
    ...
    <button class='btn'><?=ucwords($abbcd[2])?></button>
		<button class="btn btn-danger colordelete " sizeid="<?=$abbcd[0]?>">X
				...
				if(isset($_REQUEST['submit']))
				{
					...
					$i = iud("Update  `product` Set `title`='$title', `p_url`='$p_url', `category`='$category', `sub_category`='$sub_category', `price`='$price', `cut_price`='$cut_price', `discount`='$dp',`description`='$description' where id='$pid'");
					$errors = array();
				$uploadedFiles = array();
				...
				if($UploadOk == true){
						...
						$ru=select("select * from picture where product_id='$pid' and image='$name'");
    ...
    {
	$p=iud("INSERT INTO `picture`( `product_id`, `image`) VALUES ('$pid','$name')");
    }				
